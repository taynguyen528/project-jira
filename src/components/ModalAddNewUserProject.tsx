  ...
  const handleButtonAdd = async (userId: number) => {
    ...
    try {
      ...
      if (res && res.statusCode === 200) {
        ...
        const addedUser = listUser.find((user) => user.userId === userId);
        if (addedUser) {
      ...
      >
        <div className="mt-[40px]">
          ...
          <div className="flex justify-around text-[25px] px-[20px]">
            ...
            >
              <ul>
                {filteredNonProjectMembers.map((user, index) => (
                  <li key={user.userId} className="mb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        type="primary"
                        className="mr-4"
                        onClick={() => handleButtonAdd(user.userId)}
                      >
                        Add
                      </Button>
                    </div>
                    {index < filteredNonProjectMembers.length - 1 && (
                      <hr className="my-3" />
                    )}
                  </li>
                ))}
                  ...
                  <li key={member.userId} className="mb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        ...
                        danger
                        className="mr-4"
                        onClick={() => handleButtonRemove(member.userId)}
                      >
                        Remove
                      </Button>
                    </div>
